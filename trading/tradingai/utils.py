...
def download_all_data_for_training(dp: DataProvider, config: Config) -> None:
    ...
    refresh_backtest_ohlcv_data(
        dp._exchange,
        pairs=all_pairs,
        timeframes=config["tradingai"]["feature_parameters"].get("include_timeframes"),
...
def get_required_data_timerange(config: Config) -> TimeRange:
    ...
    for auto data-download in TradingAI
    """
    ...
    indicator_periods = config["tradingai"]["feature_parameters"]["indicator_periods_candles"]
    ...
    logger.info(f'TradingAI auto-downloader using {max_period} startup candles.')

    additional_seconds = max_period * max_tf_seconds

    startts = int(
        time
        - config["tradingai"].get("train_period_days", 0) * 86400
        - additional_seconds
    )
    stopts = int(time)
...
def plot_feature_importance(model: Any, pair: str, dk: FreqaiDataKitchen,
    ...
    """
    from trading.plot.plotting import go, make_subplots, store_plot_file
    ...
    for label in models:
        ...
        label = label.replace('&', '').replace('%', '')  # escape two TradingAI specific characters
...
def record_params(config: Dict[str, Any], full_path: Path) -> None:
    ...
    run_params = {
        "tradingai": config.get('tradingai', {}),
...
def get_tb_logger(model_type: str, path: Path, activate: bool) -> Any:
    ...
    if model_type == "pytorch" and activate:
        from trading.tradingai.tensorboard import TBLogger
        return TBLogger(path, activate)
    else:
        from trading.tradingai.tensorboard.base_tensorboard import BaseTensorboardLogger
