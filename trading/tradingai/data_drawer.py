...
from trading.data.history import load_pair_history
...
class FreqaiDataDrawer:
    ...
    def __init__(self, full_path: Path, config: Config):
        ...
        self.config = config
        self.tradingai_info = config.get("tradingai", {})
        ...
        self.empty_pair_dict: pair_info = {
                ...
                "data_path": "", "extras": {}}
        self.model_type = self.tradingai_info.get('model_save_type', 'joblib')
        ...
        if exists:
            try:
                ...
                with self.historic_predictions_bkp_path.open("rb") as fp:
                    self.historic_predictions = cloudpickle.load(fp)
                logger.warning('TradingAI successfully loaded the backup historical predictions file.')
    ...
    def append_model_predictions(self, pair: str, predictions: DataFrame,
        ...
        the tail is equivalent to the length of the dataframe that entered TradingAI from
    ...
    def purge_old_models(self) -> None:
        ...
        num_keep = self.tradingai_info["purge_old_models"]
        if not num_keep:
            return
    ...
    def load_data(self, coin: str, dk: FreqaiDataKitchen) -> Any:  # noqa: C901
        ...
        if dk.live and coin in self.model_dictionary:
            ...
            mod = importlib.import_module(
                self.model_type, self.tradingai_info['rl_config']['model_type'])
            MODELCLASS = getattr(mod, self.tradingai_info['rl_config']['model_type'])
    ...
    def update_historic_data(self, strategy: IStrategy, dk: FreqaiDataKitchen) -> None:
        ...
        """
        feat_params = self.tradingai_info["feature_parameters"]
    ...
    def load_all_pair_histories(self, timerange: TimeRange, dk: FreqaiDataKitchen) -> None:
        ...
        for pair in dk.all_pairs:
            if pair not in history_data:
                history_data[pair] = {}
            for tf in self.tradingai_info["feature_parameters"].get("include_timeframes"):
    ...
    def get_base_and_corr_dataframes(
        ...
        with self.history_lock:
            ...
            pairs = self.tradingai_info["feature_parameters"].get(
    ...
    def get_timerange_from_live_historic_predictions(self) -> TimeRange:
        ...
        if not self.historic_predictions_path.is_file():
            raise OperationalException(
                ...
                'to run backtest with the tradingai-backtest-live-models option '
            )
