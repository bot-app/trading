...
from trading.tradingai.utils import get_tb_logger, plot_feature_importance, record_params
...
class IFreqaiModel(ABC):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.tradingai_info: Dict[str, Any] = config["tradingai"]
        self.data_split_parameters: Dict[str, Any] = config.get("tradingai", {}).get(
            "data_split_parameters", {})
        self.model_training_parameters: Dict[str, Any] = config.get("tradingai", {}).get(
            "model_training_parameters", {})
        self.identifier: str = self.tradingai_info.get("identifier", "no_id_provided")
        self.retrain = False
        self.first = True
        self.set_full_path()
        self.save_backtest_models: bool = self.tradingai_info.get("save_backtest_models", True)
        ...
        self.scanning = False
        self.ft_params = self.tradingai_info["feature_parameters"]
        self.corr_pairlist: List[str] = self.ft_params.get("include_corr_pairlist", [])
        self.keras: bool = self.tradingai_info.get("keras", False)
        if self.keras and self.ft_params.get("DI_threshold", 0):
            self.ft_params["DI_threshold"] = 0
            logger.warning("DI threshold is not configured for Keras models yet. Deactivating.")

        self.CONV_WIDTH = self.tradingai_info.get('conv_width', 1)
        ...
        self.continual_learning = self.tradingai_info.get('continual_learning', False)
        ...
        self.activate_tensorboard: bool = self.tradingai_info.get('activate_tensorboard', True)
    ...
    def assert_config(self, config: Config) -> None:
        ...
        if not config.get("tradingai", {}):
            raise OperationalException("No tradingai parameters found in configuration file.")
    ...
    def start(self, dataframe: DataFrame, metadata: dict, strategy: IStrategy) -> DataFrame:
        ...
        # TradingAI slides the window and sequentially builds the backtesting results before returning
        # the concatenated results for the full backtesting period back to the strategy.
        else:
            self.dk = FreqaiDataKitchen(self.config, self.live, metadata["pair"])
            if not self.config.get("tradingai_backtest_live_models", False):
    ...
    def shutdown(self):
        ...
        """
        logger.info("Stopping TradingAI")
    ...
    def _start_scanning(self, strategy: IStrategy) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if retrain:
                ...
                if self.tradingai_info.get('write_metrics_to_disk', False):
    ...
    def start_backtesting(
        ...
        following the training window). TradingAI slides the window and sequentially builds
    ...
    def build_strategy_return_arrays(
        ...
        if self.tradingai_info.get('fit_live_predictions_candles', 0) and self.live:
    ...
    def check_if_feature_list_matches_strategy(
        ...
        if dk.training_features_list != feature_list:
            raise OperationalException(
                ...
                "model. In case of --strategy-list, please be aware that TradingAI "
                "requires all strategies to maintain identical "
                "feature_engineering_* functions"
            )

    def define_data_pipeline(self, threads=-1) -> Pipeline:
        ft_params = self.tradingai_info["feature_parameters"]
        pipe_steps = [
        ...
        sigma = self.tradingai_info["feature_parameters"].get('noise_standard_deviation', 0)
        if sigma:
    ...
    def set_initial_historic_predictions(
        ...
        data. After that, TradingAI will append new real predictions to the
        ...
        if self.tradingai_info['feature_parameters'].get('DI_threshold', 0) > 0:
    ...
    def fit_live_predictions(self, dk: FreqaiDataKitchen, pair: str) -> None:
        ...
        num_candles = self.tradingai_info.get("fit_live_predictions_candles", 100)
    ...
    def inference_timer(self, do: Literal['start', 'stop'] = 'start', pair: str = ''):
        """
        Timer designed to track the cumulative time spent in TradingAI for one pass through
    ...
    def track_current_candle(self):
        ...
        equivalent to the latest candle seen by TradingAI. If not, it
    ...
    def ensure_data_exists(self, len_dataframe_backtest: int,
        ...
        """
        if self.config.get("tradingai_backtest_live_models", False) and len_dataframe_backtest == 0:
    ...
    def backtesting_fit_live_predictions(self, dk: FreqaiDataKitchen):
        ...
        """
        fit_live_predictions_candles = self.tradingai_info.get("fit_live_predictions_candles", 0)
    ...
    # See tradingai/prediction_models/CatboostPredictionModel.py for an example.
    ...
    def predict(
        ...
        :do_predict: np.array of 1s and 0s to indicate places where tradingai needed to remove
    ...
    def data_cleaning_predict(self, dk: FreqaiDataKitchen, pair: str):
        ...
        if self.tradingai_info.get("DI_threshold", 0) > 0:
