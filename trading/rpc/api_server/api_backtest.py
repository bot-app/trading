...
from trading.data.btanalysis import (delete_backtest_result, get_backtest_result,
                                       get_backtest_resultlist, load_and_merge_backtest_result,
                                       update_backtest_metadata)
from trading.enums import BacktestState
...
from trading.misc import deep_merge_dicts, is_file_in_dir
from trading.rpc.api_server.api_schemas import (BacktestHistoryEntry, BacktestMetadataUpdate,
                                                  BacktestRequest, BacktestResponse)
from trading.rpc.api_server.deps import get_config
from trading.rpc.api_server.webserver_bgwork import ApiBG
from trading.rpc.rpc import RPCException
from trading.types import get_BacktestResultType_default
...
def __run_backtest_bg(btconfig: Config):
    from trading.optimize.optimize_reports import generate_backtest_stats, store_backtest_stats
...
async def api_start_backtest(
    ...
    if settings.get('tradingai', None) is not None:
        settings['tradingai'] = dict(settings['tradingai'])
...
def api_get_backtest():
    ...
    """
    from trading.persistence import LocalTrade
    if ApiBG.bgtask_running:
        return {
            "status": "running",
            "running": True,
