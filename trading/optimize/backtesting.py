...
from trading.configuration import TimeRange, validate_config_consistency
from trading.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from trading.data import history
from trading.data.btanalysis import find_existing_backtest_stats, trade_list_to_dataframe
from trading.data.converter import trim_dataframe, trim_dataframes
from trading.data.dataprovider import DataProvider
from trading.enums import (BacktestState, CandleType, ExitCheckTuple, ExitType, RunMode,
                             TradingMode)
from trading.exceptions import DependencyException, OperationalException
from trading.exchange import (amount_to_contract_precision, price_to_precision,
                                timeframe_to_minutes, timeframe_to_seconds)
from trading.exchange.exchange import Exchange
from trading.mixins import LoggingMixin
from trading.optimize.backtest_caching import get_strategy_run_id
from trading.optimize.bt_progress import BTProgress
from trading.optimize.optimize_reports import (generate_backtest_stats, generate_rejected_signals,
                                                 generate_trade_signal_candles,
                                                 show_backtest_results,
                                                 store_backtest_analysis_results,
                                                 store_backtest_stats)
from trading.persistence import LocalTrade, Order, PairLocks, Trade
from trading.plugins.pairlistmanager import PairListManager
from trading.plugins.protectionmanager import ProtectionManager
from trading.resolvers import ExchangeResolver, StrategyResolver
from trading.strategy.interface import IStrategy
from trading.strategy.strategy_wrapper import strategy_safe_wrapper
from trading.types import BacktestResultType, get_BacktestResultType_default
from trading.util.binance_mig import migrate_binance_futures_data
from trading.wallets import Wallets
    ...
    def __init__(self, config: Config, exchange: Optional[Exchange] = None) -> None:
        ...
        if self.config.get('strategy_list'):
            if self.config.get('tradingai', {}).get('enabled', False):
                logger.warning("Using --strategy-list with TradingAI REQUIRES all strategies "
        ...
        if self.config.get('tradingai_backtest_live_models', False):
            from trading.tradingai.utils import get_timerange_backtest_live_models
        ...
        if self.config.get('tradingai', {}).get('enabled', False):
            # For TradingAI, increase the required_startup to includes the training data
    ...
    def _get_close_rate_for_roi(self, row: Tuple, trade: LocalTrade, exit: ExitCheckTuple,
        ...
        if roi is not None and roi_entry is not None:
            ...
            if (trade_dur == 0 and (
                ...
                # details: https: // github.com/bot-app/trading/issues/6261
