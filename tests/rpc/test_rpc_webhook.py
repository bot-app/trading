...
from trading.enums import ExitType, RPCMessageType
from trading.rpc import RPC
from trading.rpc.discord import Discord
from trading.rpc.webhook import Webhook
...
def get_webhook_dict() -> dict:
    return {
        "enabled": True,
        "url": "https://maker.ifttt.com/trigger/trading_test/with/key/c764udvJ5jfSlswVRukZZ2/",
        "webhookentry": {
...
def test__init__(mocker, default_conf):
    ...
    webhook = Webhook(RPC(get_patched_tradingbot(mocker, default_conf)), default_conf)
...
def test_send_msg_webhook(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("trading.rpc.webhook.Webhook._send_msg", msg_mock)
    webhook = Webhook(RPC(get_patched_tradingbot(mocker, default_conf)), default_conf)
    # Test buy
    msg_mock = MagicMock()
    mocker.patch("trading.rpc.webhook.Webhook._send_msg", msg_mock)
...
def test__send_msg(default_conf, mocker, caplog):
    ...
    msg = {'value1': 'DEADBEEF',
           ...
           'value3': 'FREQTRADE'}
    post = MagicMock()
    mocker.patch("trading.rpc.webhook.post", post)
...
def test_send_msg_discord(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("trading.rpc.webhook.Webhook._send_msg", msg_mock)
    discord = Discord(RPC(get_patched_tradingbot(mocker, default_conf)), default_conf)
