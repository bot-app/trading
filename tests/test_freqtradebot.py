...
from trading.constants import CANCEL_REASON, UNLIMITED_STAKE_AMOUNT
from trading.enums import (CandleType, ExitCheckTuple, ExitType, RPCMessageType, RunMode,
...
from trading.util.datetime_helpers import dt_now, dt_utc
from trading.worker import Worker
from tests.conftest import (EXMS, create_mock_trades, create_mock_trades_usdt,
                            get_patched_tradingbot, get_patched_worker, log_has, log_has_re,
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    rpc_mock = mocker.patch('trading.tradingbot.RPCManager.send_msg', MagicMock())
    return rpc_mock
...
def test_tradingbot_state(mocker, default_conf_usdt, markets) -> None:
...
def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    trading = get_patched_tradingbot(mocker, default_conf_usdt)
    trading.process()
    assert trading.rpc.process_msg_queue.call_count == 1
...
def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    ...
    trading.config['cancel_open_orders_on_exit'] = True
    trading.cleanup()
    assert coo_mock.call_count == 1
...
def test_bot_cleanup_db_errors(mocker, default_conf_usdt, caplog) -> None:
    ...
    trading.emc.shutdown = MagicMock()
    trading.cleanup()
    assert trading.emc.shutdown.call_count == 1
...
def test_check_available_stake_amount(
    ...
    for i in range(0, max_open):
        ...
        if expected[i] is not None:
            ...
            trading.execute_entry('ETH/USDT', result)
        else:
            with pytest.raises(DependencyException):
                trading.wallets.get_trade_stake_amount('ETH/USDT')
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert trading.wallets.get_trade_stake_amount(
        'LTC/BTC', trading.edge) == (999.9 * 0.5 * 0.01) / 0.21
...
def test_edge_overrides_stoploss(limit_order, fee, caplog, mocker,
    ...
    assert trading.handle_trade(trade) is not ignore_strat_sl
...
def test_create_trade_minimal_amount(
    ...
    if create:
        assert trading.create_trade('ETH/USDT')
        if amount_enough:
        ...
        if not max_open_trades:
            assert trading.wallets.get_trade_stake_amount('ETH/USDT', trading.edge) == 0
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    trading = get_patched_tradingbot(mocker, default_conf_usdt)
    trading.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf"))
    create_mock_trades(fee, is_short)
    trading.handle_protections('ETC/BTC', '*')
    send_msg_mock = trading.rpc.send_msg
...
def test_create_trades_preopen(default_conf_usdt, ticker_usdt, fee, mocker,
    ...
    trading.execute_entry('ETH/USDT', default_conf_usdt['stake_amount'])
    trading.execute_entry('NEO/BTC', default_conf_usdt['stake_amount'])
    ...
    assert trading.create_trade('ETH/USDT')
    assert trading.create_trade('NEO/BTC')
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.trading)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.trading.state == State.RUNNING

    worker._process_running()
    assert worker.trading.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(default_conf_usdt, ticker_usdt, limit_buy_order_usdt,
    ...
    assert pair not in trading.active_pair_whitelist
    trading.process()
    assert pair in trading.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(trading.active_pair_whitelist) == len(set(trading.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch.multiple(
        'trading.strategy.interface.IStrategy',
...
def test_execute_entry(mocker, default_conf_usdt, fee, limit_order,
    ...
    assert not trading.execute_entry(pair, stake_amount, is_short=is_short)
    assert enter_rate_mock.call_count == 1
    assert enter_mm.call_count == 0
    assert trading.strategy.confirm_trade_entry.call_count == 1
    enter_rate_mock.reset_mock()

    open_order['id'] = '22'
    trading.strategy.confirm_trade_entry = MagicMock(return_value=True)
    assert trading.execute_entry(pair, stake_amount)
    assert enter_rate_mock.call_count == 1
    ...
    open_order['id'] = '33'
    fix_price = 0.06
    assert trading.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    assert trading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    trading.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert trading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order['id'] = '557'
    trading.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert trading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order['id'] = '66'
    mocker.patch(f'{EXMS}.create_order', MagicMock(return_value=order))
    assert not trading.execute_entry(pair, stake_amount)
    assert trading.strategy.leverage.call_count == 0 if trading_mode == 'spot' else 2
    ...
    with pytest.raises(PricingError, match="Could not determine entry price."):
        trading.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f'{EXMS}.get_rate', return_value=0.50)
    order['status'] = 'open'
    order['id'] = '5566'
    trading.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert trading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order['status'] = 'open'
    order['id'] = '5567'
    trading.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order['status'] = 'open'
    order['id'] = '5568'
    trading.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert trading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    trading.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)

    assert trading.execute_entry(pair, 2000, is_short=is_short)
    ...
    trading.strategy.leverage.reset_mock()
    assert trading.execute_entry(pair, 200, leverage_=3)
    assert trading.strategy.leverage.call_count == 0
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    trading.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert trading.execute_entry(pair, stake_amount)

    limit_order[entry_side(is_short)]['id'] = '222'
    trading.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert trading.execute_entry(pair, stake_amount)
...
def test_execute_entry_min_leverage(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    stake_amount = 2
    pair = 'SOL/BUSD:BUSD'
    trading.strategy.leverage = MagicMock(return_value=5.0)
...
def test_handle_stoploss_on_exchange(mocker, default_conf_usdt, fee, caplog, is_short,
    ...
    assert trading.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    assert trade.stoploss_order_id == "13434334"
    ...
    assert trading.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    trading.handle_stoploss_on_exchange(trade)
    ...
    trading.config['trailing_stop'] = True
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    trading.create_stoploss_order(trade, 200)
    assert trade.stoploss_order_id is None
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    trading.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    trading.strategy.order_types['stoploss_on_exchange_interval'] = 60
    ...
    assert trading.handle_trade(trade) is False
    assert trading.handle_stoploss_on_exchange(trade) is False
    ...
    trading.strategy.order_types['stoploss_on_exchange_interval'] = 0
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair='ETH/USDT',
        order_types=trading.strategy.order_types,
    ...
    assert trading.handle_trade(trade) is True
    assert trade.stoploss_order_id is None
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    trading.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    trading.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    trading.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    trading.strategy.stoploss = -0.02
    ...
    trading.active_pair_whitelist = trading.edge.adjust(trading.active_pair_whitelist)
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    assert not trading.update_trade_state(trade, None)
...
def test_update_trade_state_withorderdict(
    ...
    trading.update_trade_state(trade, order_id, order)
...
def test_update_trade_state_sell(
    ...
    mocker.patch('trading.wallets.Wallets.update', wallet_mock)
    ...
    trading.update_trade_state(trade, trade.open_order_id, l_order)
...
def test_handle_trade(
    ...
    patch_get_signal(trading, enter_long=False, exit_short=is_short,
...
def test_handle_overlapping_signals(
    ...
    if is_short:
        ...
        patch_get_signal(trading, enter_long=True, exit_long=True)
    trading.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(trading, enter_long=False)
    assert trading.handle_trade(trades[0]) is False
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    nb_trades = len(trades)
    ...
    for trade in trades:
        trade.is_short = is_short
    assert trading.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(default_conf_usdt, ticker_usdt, limit_order_open, fee, mocker, caplog,
    ...
    caplog.clear()
    patch_get_signal(trading)
    assert trading.handle_trade(trade)
...
def test_handle_trade_use_exit_signal(
    ...
    patch_get_signal(trading, enter_long=False, exit_long=False)
    assert not trading.handle_trade(trade)
...
def test_manage_open_orders_entry_usercustom(
    ...
    trading.strategy.check_entry_timeout = MagicMock(return_value=False)
    trading.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    ...
    nb_trades = len(trades)
    assert nb_trades == 1
    assert trading.strategy.check_entry_timeout.call_count == 1
    trading.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    assert nb_trades == 1
    assert trading.strategy.check_entry_timeout.call_count == 1
    trading.strategy.check_entry_timeout = MagicMock(return_value=True)
...
def test_manage_open_orders_entry(
    ...
    trading.strategy.check_entry_timeout = MagicMock(return_value=False)
    trading.strategy.adjust_entry_price = MagicMock(return_value=1234)
    ...
    assert trading.strategy.check_entry_timeout.call_count == 0
...
def test_adjust_entry_cancel(
    ...
    trading.strategy.ft_check_timed_out = MagicMock(return_value=False)

    # check that order is cancelled
    trading.strategy.adjust_entry_price = MagicMock(return_value=None)
    trading.manage_open_orders()
    trades = Trade.session.scalars(
        select(Trade).filter(Trade.open_order_id.is_(open_trade.open_order_id))).all()
    assert len(trades) == 0
    ...
    assert trading.strategy.adjust_entry_price.call_count == 1
...
def test_adjust_entry_maintain_replace(
    ...
    trading.strategy.adjust_entry_price = MagicMock(return_value=old_order['price'])
    trading.manage_open_orders()
    trades = Trade.session.scalars(
        select(Trade).filter(Trade.open_order_id.is_(open_trade.open_order_id))).all()
    assert len(trades) == 1
    ...
    trading.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    trading.strategy.adjust_entry_price = MagicMock(return_value=1234)
    trading.manage_open_orders()
    trades = Trade.session.scalars(
        select(Trade).filter(Trade.open_order_id.is_(open_trade.open_order_id))).all()
...
def test_manage_open_orders_exit_usercustom(
    ...
    trading.strategy.check_exit_timeout = MagicMock(return_value=False)
    trading.strategy.check_entry_timeout = MagicMock(return_value=False)
    # Return false - No impact
    trading.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 1
    assert trading.strategy.check_exit_timeout.call_count == 1
    assert trading.strategy.check_entry_timeout.call_count == 0

    trading.strategy.check_exit_timeout = MagicMock(side_effect=KeyError)
    trading.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    trading.strategy.check_exit_timeout = MagicMock(return_value=True)
    trading.strategy.check_entry_timeout = MagicMock(return_value=True)
    trading.manage_open_orders()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 2
...
def test_manage_open_orders_exit(
    ...
    assert trading.strategy.check_exit_timeout.call_count == 0
    assert trading.strategy.check_entry_timeout.call_count == 0
...
def test_manage_open_orders_partial_fee(
    ...
    mocker.patch('trading.wallets.Wallets.get_free', MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    assert trading.handle_cancel_enter(trade, l_order, reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order['filled'] = 0.01
    assert not trading.handle_cancel_enter(trade, l_order, reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not trading.handle_cancel_enter(trade, limit_order[entry_side(is_short)], reason)
...
def test_handle_cancel_enter_exchanges(mocker, caplog, default_conf_usdt, is_short, fee,
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert trading.handle_cancel_enter(trade, limit_buy_order_canceled_empty, reason)
    assert cancel_order_mock.call_count == 0
    assert log_has_re(
...
def test_handle_cancel_exit_limit(mocker, default_conf_usdt, fee) -> None:
    ...
    assert trading.handle_cancel_exit(trade, order, reason)
    assert cancel_order_mock.call_count == 1
    ...
    order['amount'] = 2
    order['filled'] = 1.9
    assert not trading.handle_cancel_exit(trade, order, reason)
...
def test_execute_trade_exit_up(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_up, mocker,
    ...
    assert id(trading.strategy.confirm_trade_exit.call_args_list[0][1]['trade']) != id(trade)
    assert trading.strategy.confirm_trade_exit.call_args_list[0][1]['trade'].id == trade.id
...
def test_execute_trade_exit_custom_exit_price(
    ...
    trading.strategy.custom_exit_price = lambda **kwargs: 2.25
    trading.execute_trade_exit(
        trade=trade,
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    mocker.patch('trading.wallets.Wallets.get_free', MagicMock(return_value=300))
    create_order_mock = MagicMock(side_effect=[
        {'id': '12345554'},
        {'id': '12345555'},
    ])
    ...
    trading.config['dry_run'] = False
    trade.stoploss_order_id = "abcd"

    trading.execute_trade_exit(trade=trade, limit=1234,
                                 exit_check=ExitCheckTuple(exit_type=ExitType.STOP_LOSS))
    assert create_order_mock.call_count == 2
...
def test_execute_trade_exit_market_order(
    ...
    trading.config['order_types']['exit'] = 'market'
...
def test_execute_trade_exit_insufficient_funds_error(default_conf_usdt, ticker_usdt, fee, is_short,
    ...
    assert not trading.execute_trade_exit(
        trade=trade,
        limit=ticker_usdt_sell_up()['ask' if is_short else 'bid'],
...
def test_exit_profit_only(
    ...
    if exit_type == ExitType.EXIT_SIGNAL.value:
        ...
        trading.strategy.ft_stoploss_reached = MagicMock(return_value=ExitCheckTuple(
    ...
    if profit_only:
        ...
        trading.strategy.custom_exit.call_count == 1

    patch_get_signal(trading, enter_long=False, exit_short=is_short, exit_long=not is_short)
    assert trading.handle_trade(trade) is handle_first

    if handle_second:
        trading.strategy.exit_profit_offset = 0.0
        assert trading.handle_trade(trade) is True
...
def test_sell_not_enough_balance(default_conf_usdt, limit_order, limit_order_open,
    ...
    trade.update_trade(oobj)
    patch_get_signal(trading, enter_long=False, exit_long=True)
    mocker.patch('trading.wallets.Wallets.get_free', MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch('trading.wallets.Wallets.get_free', MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch('trading.wallets.Wallets.update')
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to exit trade."):
            assert trading._safe_exit_amount(trade, trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert trade.amount != amount_wallet
        assert trading._safe_exit_amount(trade, trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r'.*Falling back to wallet-amount.', caplog)
        assert trade.amount == amount_wallet
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert trading._safe_exit_amount(trade, trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(default_conf_usdt, ticker_usdt, fee,
    ...
    assert trading.strategy.is_pair_locked(trade.pair, side='*')
    # Boths sides are locked
    assert trading.strategy.is_pair_locked(trade.pair, side='long')
    assert trading.strategy.is_pair_locked(trade.pair, side='short')
...
def test_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open, is_short,
    ...
    if is_short:
        patch_get_signal(trading, enter_long=False, exit_short=False, exit_tag='something')
    else:
        patch_get_signal(trading, enter_long=False, exit_long=False, exit_tag='something')
    assert trading.handle_trade(trade) is True
...
def test_disable_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open,
    ...
    patch_get_signal(trading, enter_long=not is_short, enter_short=is_short, exit_short=is_short)
    assert trading.handle_trade(trade) is True
...
def test_get_real_amount_quote(default_conf_usdt, trades_for_order, buy_order_fee, fee, caplog,
    ...
    assert trading.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
def test_get_real_amount_quote_dust(default_conf_usdt, trades_for_order, buy_order_fee, fee,
                                    caplog, mocker):
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=trades_for_order)
    walletmock = mocker.patch('trading.wallets.Wallets.update')
    mocker.patch('trading.wallets.Wallets.get_free', return_value=8.1122)
    amount = sum(x['amount'] for x in trades_for_order)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    assert trading.get_real_amount(trade, buy_order_fee, order_obj) is None
...
def test_get_real_amount(
    ...
    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = trading.get_real_amount(trade, buy_order, order_obj)
...
def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    assert trading.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            'Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
...
def test_get_real_amount_invalid_order(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    assert trading.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
def test_get_real_amount_fees_order(default_conf_usdt, market_buy_order_usdt_doublefee,
    ...
    assert trading.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
...
def test_get_real_amount_wrong_amount(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        trading.get_real_amount(trade, limit_buy_order_usdt, order_obj)
...
def test_get_real_amount_wrong_amount_rounding(default_conf_usdt, trades_for_order, buy_order_fee,
    ...
    assert pytest.approx(trading.get_real_amount(
...
def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert trading.get_real_amount(trade, order, order_obj) is None
...
def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = trading.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
    ...
    caplog.clear()
    trading.config['exchange']['unknown_fee_rate'] = 1
    res = trading.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
def test_apply_fee_conditional(default_conf_usdt, fee, mocker, caplog,
    ...
    walletmock = mocker.patch('trading.wallets.Wallets.update')
    mocker.patch('trading.wallets.Wallets.get_free', return_value=wallet)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert trading.apply_fee_conditional(trade, 'LTC', amount, fee_abs, order) == amount_exp
    assert walletmock.call_count == 1
...
def test_apply_fee_conditional_multibuy(default_conf_usdt, fee, mocker, caplog,
    ...
    assert trading.apply_fee_conditional(trade, 'LTC', amount, fee_abs, order1) == amount_exp
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
def test_order_book_entry_pricing1(mocker, default_conf_usdt, order_book_l2, exception_thrown,
    ...
    if exception_thrown:
        ...
        assert trading.exchange.get_rate(
...
def test_startup_trade_reinit(default_conf_usdt, edge_conf, mocker):
    ...
    mocker.patch('trading.persistence.Trade.stoploss_reinitialization', reinit_mock)
    ...
    ftbot = get_patched_tradingbot(mocker, edge_conf)
    ftbot.startup()
...
def test_sync_wallet_dry_run(mocker, default_conf_usdt, ticker_usdt, fee, limit_buy_order_usdt_open,
    ...
    bot = get_patched_tradingbot(mocker, default_conf_usdt)
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    ...
    trading.check_for_open_trades()
    assert trading.rpc.send_msg.call_count == 0
    ...
    trading.check_for_open_trades()
    assert trading.rpc.send_msg.call_count == 1
    assert 'Handle these trades manually' in trading.rpc.send_msg.call_args[0][0]['status']
...
def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    ...
    trading.startup_backpopulate_precision()
    trades = Trade.get_trades().all()
    for trade in trades:
...
def test_update_trades_without_assigned_fees(mocker, default_conf_usdt, fee, is_short):
    ...
    trading.update_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    trading.handle_insufficient_funds(trades[3])
    ...
    trading.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    trading.handle_insufficient_funds(trades[4])
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    trading = get_patched_tradingbot(mocker, default_conf_usdt)
    mock_uts = mocker.spy(trading, 'update_trade_state')
    ...
    Trade.session.add(trade)
    trading.handle_onexchange_order(trade)
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    valid_price_from_string = trading.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = trading.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = trading.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = trading.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = trading.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = trading.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_update_funding_fees_schedule(mocker, default_conf, trading_mode, calls, time_machine,
    ...
    # Check schedule jobs in debugging with trading._schedule.jobs
    trading._schedule.run_pending()

    assert trading.update_funding_fees.call_count == calls
...
def test_update_funding_fees(
    ...
    trading.execute_entry('ETH/USDT', 123, is_short=is_short)
    trading.execute_entry('LTC/USDT', 2.0, is_short=is_short)
    trading.execute_entry('XRP/USDT', 123, is_short=is_short)
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    assert trading.execute_entry(pair, stake_amount, trade=trade)
    ...
    assert trading.execute_trade_exit(trade=trade, limit=8,
...
def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 MagicMock(return_value=closed_successful_buy_order))
    assert trading.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 MagicMock(return_value=closed_sell_dca_order_1))
    assert trading.execute_trade_exit(trade=trade, limit=ask,
                                        exit_check=ExitCheckTuple(exit_type=ExitType.PARTIAL_EXIT),
...
def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    ...
    for idx, (order, result) in enumerate(data):
        ...
        if order[0] == 'buy':
            assert trading.execute_entry(pair, amount, trade=trade)
        else:
            assert trading.execute_trade_exit(
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    trading.process_open_trade_positions()
...
def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    create_mock_trades(fee)
    caplog.set_level(logging.DEBUG)
    trading.strategy.adjust_trade_position = MagicMock(return_value=10)
    trading.process_open_trade_positions()
